...
```
usage: md_embed <tag_file> <policy_dir> <elf_file>
```

* `tag_file` is the name of tag info file to read.
* `policy_dir` is the directory into which the policy was generated by the policy tool.
* `elf_file` is the name of the elf to embed tagging information into.

This utility takes the existing tag information and embeds it into the elf file
in a section called `.initial_tag_map`. This section can be used by a loader to
tag dynamically loaded code.
