...
void finite_pipe_t::install_rule(operands_t *ops, results_t *res) {
  ...
  if (entry_used[next_entry]==true) {
    ...
    if ((existing_entry==pipe_table.end())) {
      ...
      pipe_table.erase(existing_entry);
    }
  }
  pipe_table.insert(std::make_pair(*ops_copy, *res_copy));
  /**printf("Table contents:\n");
  for (auto it = pipe_table.begin(); it != pipe_table.end(); it++) {
...
bool finite_pipe_t::allow(operands_t *ops, results_t *res) {
  /**printf("Table contents:\n");
  for (auto it = pipe_table.begin(); it != pipe_table.end(); it++) {
    printf("pc: %" PRIu32 ", ci: %" PRIu32, it->first.pc->tags[0], it->first.ci->tags[0]);
    if (it->first.op1) printf(", op1: %" PRIu32, it->first.op1->tags[0]);
