...
public:
  ...
  bool lookup(meta_set_t *ops, meta_set_t *res);
  void compute_hashes(meta_set_t *ops, int *hashes);

private:
  void copy_victim_to_reinsert();
  void insert(int address, meta_set_t *ops, meta_set_t *res, int hops, int *do_not_victimize);
  void real_insert(int address, meta_set_t *ops, meta_set_t * res, int *hashes, int free_slot);
  bool hit(meta_set_t *ops,int address);
  void evict_mtable_entry(int address, meta_set_t *victim_ops, meta_set_t *victim_res, int *do_not_victimize);
  void check_direct_k_hash_conflicts(meta_set_t *ops); // goes with DMHC_DIRECT_K_HASH_CONFLICTS
